...
static auto PrepareSplit(const Shape& shape, int axis, const std::string& name) {
...
std::vector<Variable> Split(const Variable& x, int n_section, int axis) {
    ...
    return FuncImpl<SplitNSecSubset>(n_section, std::get<0>(ps), std::get<1>(ps))({x});
}

std::vector<Variable> Split(const Variable& x, const Index& idxs, int axis) {
    auto&& ps = PrepareSplit(x.shape(), axis, "Split");
    return FuncImpl<SplitIdxsSubset>(idxs, std::get<0>(ps), std::get<1>(ps))({x});
}
